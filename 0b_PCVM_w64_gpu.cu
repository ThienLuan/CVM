    ...
    for (int i = 0; i < l_par; i++) {
       t_shift = i%l_par;  
        ...
        if (g_input_string[ start_tid+i ] == 'A') {
            bit_vector = bit_vector + ((g_pattern_decode[0] >> t_shift*b) & mask);
        }
        else if (g_input_string[ start_tid+i ] == 'C'){
            bit_vector = bit_vector + ((g_pattern_decode[1] >> t_shift*b) & mask);
        }
        else if (g_input_string[ start_tid+i ] == 'T'){
            bit_vector = bit_vector + ((g_pattern_decode[2] >> t_shift*b) & mask);
        }
        else if (g_input_string[ start_tid+i ] == 'G'){   //case of G
            bit_vector = bit_vector + ((g_pattern_decode[3] >> t_shift*b) & mask);
        }
        else {  // can be char "\n" 
...
//        }
	   ...
	   for (int j = 0; j < (real_pattern_size-l_par+1) ; j++) {  //circular patterns
                h_matched_result[i*(real_pattern_size-l_par+1)+j] = bit_vector % pow_2b;
                bit_vector = bit_vector >> b;
	   }
    }// for h_input_string
...
*/
    ...
    printf("#----------------------------------------------------------#\n");
    printf("#---------------Proposed PCVM Alg. in GPU------------------#\n");
    printf("############################################################\n");
