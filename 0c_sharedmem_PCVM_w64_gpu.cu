...
__global__ void ASM_kernel(char *g_input_string, int input_size, int *g_pattern_decode, int real_pattern_size, int l_par, int mask, int maskplus, int b, int *g_matched_result)
{
    int tid  = threadIdx.x ;
  ...
  if ( (l_par <= tid) && (tid < l_par + 4) ){
     pattern_decode_shared[tid-l_par] = g_pattern_decode[tid-l_par];
  }
  __syncthreads();

////////////////////////////////////////////////////////////////

  if (start_tid < input_size-l_par+1) {
    for (int i = 0; i < l_par; i++) {
       t_shift = i%l_par;  

        if (sub_string_shared[ tid+i ] == 'A') {
            bit_vector = bit_vector + ((pattern_decode_shared[0] >> t_shift*b) & mask);
        }
        else if (sub_string_shared[ tid+i ] == 'C'){
            bit_vector = bit_vector + ((pattern_decode_shared[1] >> t_shift*b) & mask);
        }
        else if (sub_string_shared[ tid+i ] == 'T'){
            bit_vector = bit_vector + ((pattern_decode_shared[2] >> t_shift*b) & mask);
        }
        else if (sub_string_shared[ tid+i ] == 'G'){   //case of G
            bit_vector = bit_vector + ((pattern_decode_shared[3] >> t_shift*b) & mask);
        }
        else {  // can be char "\n" 
        ...
        //Get results
           for (int j = 0; j < (real_pattern_size-l_par+1) ; j++) {  //circular patterns
                //h_matched_result[(i-l_par+1)*l_par+(l_par-1-j)] += ((bit_vector >> (k*l_par+j)) & 1);
                g_matched_result[start_tid*(real_pattern_size-l_par+1)+j] = bit_vector % pow_2b;
                bit_vector = bit_vector >> b;
           }

    //cuPrintf("threadIdx.x = %d \t ,start = %d, matrix_M = %d, matrix_B = %d, string = %s, pattern = %s, pattern_size = %d\n", tid, start_tid, g_matrix_M[(real_pattern_size+1)*(start_tid+i) + i],g_matrix_B[(real_pattern_size+1)*(start_tid+i) + i], g_input_string[i-1], g_pattern_circular[i-1], l_par);
  }

}

////////////////////////////////
void ASM_process_top (char *g_input_string, size_t input_size,  int *g_pattern_decode, int real_pattern_size, int l_par, int mask, int maskplus, int b, int *g_matched_result)
{

    // num_blocks = # of thread blocks to cover input stream
    int num_blocks = (input_size-l_par+1)/THREAD_BLOCK_SIZE + 1 ;
    ...
    ASM_kernel <<< dimGrid, dimBlock >>>(g_input_string, input_size, g_pattern_decode, real_pattern_size, l_par, mask, maskplus, b, g_matched_result);
...
//    cudaGetDeviceProperties(&deviceProp, deviceID);
    ...
    h_matched_result = (int *) malloc (sizeof(int)*(input_size-l_par+1)*(real_pattern_size-l_par+1));
    assert( NULL != h_matched_result );
    memset( h_matched_result, 0, sizeof(int)*(input_size-l_par+1)*(real_pattern_size-l_par+1)) ;
...
float sub_l_par = l_par+1;
float float_b = log2(sub_l_par);
int b = float_b;
if(b != float_b) {
   b=b+1;
}
//int b = log2(sub_l_par)+1;
//int pow_2b = 1 << b;
//printf("#-pattern = %d, b=%d, float_b = %f, opw_2b = %d-#\n",l_par,b,float_b,pow_2b);
    ...
    for (int i = 0; i < (real_pattern_size-l_par+1)*b ; i++) {
	mask = (mask << 1) | 1;
    }
    unsigned long long int maskplus = 0;
    for (int i = 0; i < (real_pattern_size-l_par+1) ; i++) {
...
/*
    ...
    for (int i = 0; i<input_size-(l_par-1) ; i++) {
	bit_vector = 0 ;
      for(int k=0; k<=real_pattern_size-l_par; k++) {
...
//        }
	   ...
	   for (int j = 0; j < l_par ; j++) {  //circular patterns
		//h_matched_result[(i-l_par+1)*l_par+(l_par-1-j)] += ((bit_vector >> (k*l_par+j)) & 1);
                h_matched_result[i*l_par+j] = bit_vector % pow_2b;
                bit_vector = bit_vector >> b;
	   }
    }// for h_input_string
...
*/
    ...
    cudaMalloc (&g_matched_result, sizeof(int)*(input_size-l_par+1)*(real_pattern_size-l_par+1));
    ...
    ASM_process_top ( g_input_string, input_size, g_pattern_decode, real_pattern_size, l_par, mask, maskplus, b, g_matched_result) ;
    ...
    cudaMemcpy (h_matched_result, g_matched_result, sizeof(int)*(input_size-l_par+1)*(real_pattern_size-l_par+1), cudaMemcpyDeviceToHost );


    // step 4: output matched result
    int total_result = 0;
    for (int i = 0; i < input_size-(l_par-1); i++) {
        for (int j = 0; j < (real_pattern_size-l_par+1); j++) {
            //printf("At position %4d, circular pattern %4d : match pattern %d\n", i, j, h_matched_result[i*l_par + j]);
	    if(h_matched_result[i*(real_pattern_size-l_par+1) + j] <= k_par) {total_result++;}
	}
    }
    printf("\n\n");
    printf("############################################################\n");
    ...
    printf("#----------------------------------------------------------#\n");
    printf("#----------Proposed shared-mem PCVM Alg. in GPU------------#\n");
    printf("############################################################\n");
